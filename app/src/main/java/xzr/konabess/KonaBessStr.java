...
public class KonaBessStr {
    ...
    public static String help(String what){
        if(what.equals("qcom,gpu-freq"))
            return "这是GPU的工作频率，您可以通过改变该数值来改变频率表中的可用频率。\n" +
    ...
    public static String generic_help="欢迎使用本工具！\n" +
            "通过使用本工具，您可以自由的对骁龙865系列处理器的GPU进行超频、降压等操作。\n" +
            "以下通过一个示例来说明如何超频：\n" +
            "首先点击“编辑GPU频率表”，软件将会显示设备中存在的频率表，一个设备中可能存在多个频率表。" +
            "我已对频率表按照高通默认在安卓R上的情况进行了注释，但是注释不一定准确，因此请根据各种GPU频率" +
